...
macro(BUILD_JAFAR_MODULE modulename)
  ...
  if(${NB_EXTLIBS} GREATER 0)
	...
	string(TOUPPER "${extlib}" THIS_OPT_BOOST_COMPONENENT)
	set(${THIS_MODULE_OPTIONAL_EXTLIBS_FLAGS} "${${THIS_MODULE_OPTIONAL_EXTLIBS_FLAGS}} -D${THIS_OPT_BOOST_COMPONENENT}")
      else("${LIB_EXACT_NAME}" STREQUAL "Boost")
	if(("${LIB_EXACT_NAME}" STREQUAL "QT" AND QT4_FOUND) 
	    OR ("${LIB_EXACT_NAME}" STREQUAL "QT4" AND QT4_FOUND))
	  set(QT_WRAPPING_REQUIRED 1)
	  set(${THIS_MODULE_OPTIONAL_EXTLIBS_FLAGS} "${${THIS_MODULE_OPTIONAL_EXTLIBS_FLAGS}} -DQT4")
	  list(APPEND THIS_PROJECT_DEPENDS_ALL_LIST ${QT_LIBRARIES})
	  ...
	  if(${THIS_LIB_FOUND})
	    set(LIB_EXACT_NAME_LIBRARIES "${LIB_EXACT_NAME}_LIBRARIES")
	    list(APPEND THIS_PROJECT_DEPENDS_ALL_LIST ${${LIB_EXACT_NAME_LIBRARIES}})
	    string(TOUPPER "${extlib}" THIS_OPT_LIB_NAME)
	    set(${THIS_MODULE_OPTIONAL_EXTLIBS_FLAGS} "${${THIS_MODULE_OPTIONAL_EXTLIBS_FLAGS}} -D${THIS_OPT_LIB_NAME}")
	  else(${EXTLIB_FOUND})
  ...
  set(ALL_COMPILER_FLAGS ${THIS_MODULE_CXXFLAGS} ${THIS_MODULE_CPPFLAGS} "-D_JFR_MODULE_=\\\"${MODULENAME}\\\"" "${${THIS_MODULE_OPTIONAL_MODULES_FLAGS}}" "${${THIS_MODULE_OPTIONAL_EXTLIBS_FLAGS}}")
