...
macro(BUILD_JAFAR_MODULE modulename)
  ...
  if(${NB_EXTLIBS} GREATER 0)
    foreach(extlib ${THIS_MODULE_REQUIRED_EXTLIBS})
      get_ext_library_exact_name(${extlib} LIB_EXACT_NAME COMPONENT_NAME)
			if(NOT "${COMPONENT_NAME}" STREQUAL "")
				list(APPEND ${LIB_EXACT_NAME}_COMPONENTS ${COMPONENT_NAME})
				list(APPEND EXTLIBS_COMPONENTS ${LIB_EXACT_NAME}_COMPONENTS)
			endif(NOT "${COMPONENT_NAME}" STREQUAL "")
      ...
      if("${LIB_EXACT_NAME}" STREQUAL "Boost")
				get_component_library("Boost" ${extlib} BOOST_COMPONENT_LIBRARY)
  ...
  set(COMMON_COMPILER_FLAGS "${JAFAR_COMPILE_FLAGS_MODULE} ${THIS_MODULE_CXXFLAGS} ${THIS_MODULE_CPPFLAGS} -D_JFR_MODULE_=\\\"${MODULENAME}\\\"")
...
#    OUTPUT_FILE ${CMAKE_INSTALL_PREFIX}/modules/${modulename}/data)
  ...
  if(ENABLE_TCL)
	...
	if(NOT "${EXTLIBS_COMPONENTS}" STREQUAL "")
		list(REMOVE_DUPLICATES EXTLIBS_COMPONENTS)
		foreach(ext_with_components ${EXTLIBS_COMPONENTS})
			list(REMOVE_DUPLICATES ${ext_with_components})
			message(STATUS "${ext_with_components}=${${ext_with_components}}")
			set(COMPONENTS_ARGS "${COMPONENTS_ARGS} -D${ext_with_components}=\"")
			message("${ext_with_components} ${${ext_with_components}}")
			foreach(component ${${ext_with_components}})
				set(COMPONENTS_ARGS "${COMPONENTS_ARGS}${component};")
			endforeach(component)
			set(COMPONENTS_ARGS "${COMPONENTS_ARGS}\"")
		endforeach(ext_with_components)
		message(STATUS "${COMPONENTS_ARGS}")
	endif(NOT "${EXTLIBS_COMPONENTS}" STREQUAL "")
	#Voodoo strip for cmake 2.8.3
	string(STRIP "${COMPONENTS_ARGS}" COMPONENTS_ARGS)
  add_custom_target(package-${modulename}
    ${CMAKE_COMMAND} -DMODULENAME=${MODULENAME} -DJafar_SOURCE_DIR=${Jafar_SOURCE_DIR} -DJafar_BINARY_DIR=${Jafar_BINARY_DIR} -DTHIS_MODULE_SOURCE_DIR=${CMAKE_CURRENT_SOURCE_DIR} -DTHIS_MODULE_FULL_VERSION=${THIS_MODULE_FULL_VERSION} -DTHIS_MODULE_VERSION=${THIS_MODULE_VERSION} -DTHIS_MODULE_ROBOTPKG_CATEGORY=${THIS_MODULE_ROBOTPKG_CATEGORY} -DTHIS_MODULE_BINARY_DIR=${CMAKE_CURRENT_BINARY_DIR} -DTHIS_MODULE_REQUIRED_MODULES="${THIS_MODULE_REQUIRED_MODULES}" -DTHIS_MODULE_OPTIONAL_MODULES="${THIS_MODULE_OPTIONAL_MODULES}" -DTHIS_MODULE_ROBOTPKG_REQ_MODULES="${THIS_MODULE_ROBOTPKG_REQ_MODULES}" -DTHIS_MODULE_ROBOTPKG_OPT_MODULES="${THIS_MODULE_ROBOTPKG_OPT_MODULES}" -DTHIS_MODULE_REQUIRED_EXTLIBS_EXACT_NAMES="${THIS_MODULE_REQUIRED_EXTLIBS_EXACT_NAMES}" -DTHIS_MODULE_OPTIONAL_EXTLIBS_EXACT_NAMES="${THIS_MODULE_OPTIONAL_EXTLIBS_EXACT_NAMES}" -DTHIS_MODULE_ROBOTPKG_REQ_EXTLIBS="${THIS_MODULE_ROBOTPKG_REQ_EXTLIBS}" -DTHIS_MODULE_ROBOTPKG_OPT_EXTLIBS="${DTHIS_MODULE_ROBOTPKG_OPT_EXTLIBS}" -DCMAKE_SHARED_LIBRARY_PREFIX=${CMAKE_SHARED_LIBRARY_PREFIX} -DCMAKE_SHARED_LIBRARY_SUFFIX=${CMAKE_SHARED_LIBRARY_SUFFIX} -DGIT_EXECUTABLE=${GIT_EXECUTABLE} -DCMAKE_SYSTEM_NAME=${CMAKE_SYSTEM_NAME} -DTHIS_MODULE_SHORT_DESCRIPTION=${THIS_MODULE_SHORT_DESCRIPTION} -DTHIS_MODULE_ROBOTPKG_OPT_EXTLIBS=${THIS_MODULE_ROBOTPKG_OPT_EXTLIBS} -DTHIS_MODULE_EXTLIBS_COMPONENTS=${EXTLIBS_COMPONENTS} "${COMPONENTS_ARGS}" -P ${Jafar_SOURCE_DIR}/tools/cmake/JafarModuleRelease.cmake)
