...
namespace jafar {
  namespace slam {
	...
	case POINT_POLAR:
	  return 3;
	case POINT_STEREOIMAGE:
	  return 3;
	case POINT_BEARING:
	  return 2;
	case POINT_IMAGE:
	  return 2;
	case POINT_OMNIIMAGE:
	  return 2;
	case SEGMENT_IMAGE:
	  return 2;
        case SEGMENTID_IMAGE:
            return 2;
        case SEGMENTID_EXT_IMAGE:
            return 4;
        case SEGMENT_STEREOIMAGE:
            return 4;
        case BASIS:
            return 6;
        case ROBOT:
          return 6;
	default:
	  JFR_RUN_TIME("Undefined observation size for type: " << ot);
	}
      };

      ObservationType type;

      /// The id of the sensor (it's a unique number associated to an unique sensor, it's up to the 
      /// user to choose the numbers)
      int sensorId;
      ...
      void set(unsigned int id_,	       
	       const jblas::vec& z_,
	       unsigned int sensorId_ = 0) {
	JFR_PRECOND(z_.size() == z.size(),
		    "Observation::set: invalid size ");
	sensorId = sensorId_;
	id = id_;
	z.assign(z_);
      }
    void set(unsigned int id_,        
           const jblas::vec& z_,
           unsigned int sensorId_, unsigned int robotId_) {
    JFR_PRECOND(z_.size() == z.size(),
            "Observation::set: invalid size ");
    sensorId = sensorId_;
    robotId = robotId_;
